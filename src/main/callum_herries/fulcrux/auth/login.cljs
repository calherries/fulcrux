(ns callum-herries.fulcrux.auth.login
  (:require
    ["react" :as react]
...
(defsc Login [this {:user/keys [email]
  ...
  {:query              [:ui/open? :ui/error :user/email
                        session-join
                        [::sm/asm-id :callum-herries.fulcrux.client.ui.root/TopRouter]
     ...
     (when-not on-start-state?
       (if logged-in?
         [:button.item
          ...
          ;; TEMPLATE CHANGED
          #_(fu/hover-hand nil (str current-user ent/nbsp "Log out"))]
         ...
         [:<>
          ...
          [:div {:className "item" :key "login" :onClick #(toggle-modal! this)}
           ;; TEMPLATE CHANGED
           #_(fu/hover-hand #js{:key "login-label"} "Login")
           (when open?
             ;; TEMPLATE CHANGED
             [:div "EMPTY - TEMPLATE CHANGED"]
             #_(ui-floating-menu
